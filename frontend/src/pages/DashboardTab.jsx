...
// }
  ...
  import bl from "../assets/bucketlist.png";
  import game from "../assets/game-tord.png"
  import alpha from "../assets/datees.png"

// frontend/src/pages/DashboardTab.jsx
export default function DashboardTab({ dates, buckets, memories, scores, user, partner, setTab, theme }) {
  const totalDone  = Object.values(dates).filter(d => d.done).length;
  const bucketDone = buckets.filter(b => b.done).length;
  const p1Name     = user?.name?.split(" ")[0]    || "You";
  const p2Name     = partner?.name?.split(" ")[0] || "Partner";
  ...
  const tertiary   = theme?.tertiary  || "#f97316";

  const QUICK = [
    { id:"dates",       emoji: alpha, label:"Dates",   color:"#f43f5e" },
    { id:"betgame",     emoji:game, label:"Game",     color:"#10b981" },
    { id:"bucket",      emoji: bl, label:"Bucket",  color:"#8b5cf6" },
    { id:"memories",    emoji:game, label:"Moments", color:"#f97316" },
  ];
  ...
  return (
    <div style={{ maxWidth:"480px", margin:"0 auto", padding:"12px 12px 80px" }}>
      <style>{`
        ...
        @keyframes shimmer { 0%{background-position:200% center} 100%{background-position:-200% center} }
        @keyframes hbeat   { 0%,100%{transform:scale(1)} 50%{transform:scale(1.18)} }
        .db-hero  { animation: scaleIn 0.45s cubic-bezier(0.34,1.1,0.64,1) both; }
        .db-c1    { animation: fadeUp  0.4s cubic-bezier(0.34,1.2,0.64,1) both; animation-delay:0.05s; }
        .db-c2    { animation: fadeUp  0.4s cubic-bezier(0.34,1.2,0.64,1) both; animation-delay:0.10s; }
        .db-c3    { animation: fadeUp  0.4s cubic-bezier(0.34,1.2,0.64,1) both; animation-delay:0.15s; }
        .db-c4    { animation: fadeUp  0.4s cubic-bezier(0.34,1.2,0.64,1) both; animation-delay:0.20s; }
        .db-c5    { animation: fadeUp  0.4s cubic-bezier(0.34,1.2,0.64,1) both; animation-delay:0.25s; }
        .db-c6    { animation: fadeUp  0.4s cubic-bezier(0.34,1.2,0.64,1) both; animation-delay:0.30s; }
        .hb { animation: hbeat 1.8s ease-in-out infinite; }
        .shimmer-t {
          background: linear-gradient(90deg,${primary},${secondary},${tertiary},${primary});
          background-size:200% auto;
          -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;
          animation: shimmer 3s linear infinite;
        }
        .tap-card { transition: transform 0.18s, box-shadow 0.18s; cursor:pointer; }
        .tap-card:active { transform: scale(0.97); }
      `}</style>

      {/* ── Hero ── */}
      <div className="db-hero" style={{
        background:`linear-gradient(135deg,${primary} 0%,${secondary} 55%,${tertiary} 100%)`,
        borderRadius:"28px", padding:"20px 18px 18px",
        marginBottom:"14px", position:"relative", overflow:"hidden",
        boxShadow:`0 10px 36px ${primary}44`,
      }}>
        {/* blobs */}
        <div style={{position:"absolute",top:"-28px",right:"-18px",width:"110px",height:"110px",borderRadius:"50%",background:"rgba(255,255,255,0.09)"}}/>
        <div style={{position:"absolute",bottom:"-32px",left:"20px",width:"80px",height:"80px",borderRadius:"50%",background:"rgba(255,255,255,0.07)"}}/>

        {/* greeting row */}
        <div style={{position:"relative"}}>
          <div style={{fontSize:"11px",color:"rgba(255,255,255,0.72)",letterSpacing:"1.5px",textTransform:"uppercase",marginBottom:"14px",fontWeight:600}}>
            {greeting} 
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",marginBottom:"16px",gap:"0"}}>
            ...
            <div style={{position:"relative",zIndex:2}}>
              {user?.photo
                ? <img src={user.photo} alt="" style={{width:"56px",height:"56px",borderRadius:"50%",border:"3px solid white",objectFit:"cover",boxShadow:"0 4px 14px rgba(0,0,0,0.2)"}}/>
                : <div style={{width:"56px",height:"56px",borderRadius:"50%",border:"3px solid white",background:"rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px",fontWeight:700,color:"white"}}>{p1Name[0]}</div>
              }
            </div>
            {/* Heart */}
            <div className="hb" style={{zIndex:3,position:"relative",margin:"0 -4px",width:"28px",height:"28px",borderRadius:"50%",background:"white",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}>❤️</div>
            {/* P2 avatar */}
            <div style={{position:"relative",zIndex:2}}>
              {partner?.photo
                ? <img src={partner.photo} alt="" style={{width:"56px",height:"56px",borderRadius:"50%",border:"3px solid white",objectFit:"cover",boxShadow:"0 4px 14px rgba(0,0,0,0.2)"}}/>
                : <div style={{width:"56px",height:"56px",borderRadius:"50%",border:"3px solid white",background:"rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px",fontWeight:700,color:"white"}}>{p2Name[0]}</div>
              }
            </div>
            <div style={{marginLeft:"14px"}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:"20px",fontWeight:700,color:"white",lineHeight:1.2}}>
                {p1Name} ♥  {p2Name}
              </div>
              <div style={{fontSize:"11px",color:"rgba(255,255,255,0.75)",marginTop:"3px"}}>Your couple journey</div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"8px"}}>
            {[
              {v:`${totalDone}`,  sub:"/ 26 dates"},
              {v:`${memories.length}`, sub:"memories" },
              {v:`${bucketDone}`, sub:`/ ${buckets.length} dreams`},
              {v:`${p1Score+p2Score}`, sub:"total pts"},
            ].map((s,i) => (
              <div key={i} style={{background:"rgba(255,255,255,0.18)",borderRadius:"14px",padding:"9px 4px",textAlign:"center",backdropFilter:"blur(8px)"}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:"20px",fontWeight:700,color:"white",lineHeight:1}}>{s.v}</div>
                <div style={{fontSize:"9px",color:"rgba(255,255,255,0.72)",marginTop:"2px",lineHeight:1.2}}>{s.sub}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Quick actions ── */}
      <div className="db-c1" style={{
        background:"white",borderRadius:"22px",padding:"14px 10px",
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"4px"}}>
          {QUICK.map(q => (
            <button key={q.id} onClick={() => setTab(q.id)} style={{
              ...
              padding:"10px 4px",borderRadius:"16px",border:"none",
            ...
            >
              <div style={{
                ...
                background:`${q.color}15`,border:`1.5px solid ${q.color}22`,
                display:"flex",alignItems:"center",justifyContent:"center",fontSize:"20px",
              }}>
                <img src={q.emoji} alt={q.label}  style={{
    width: "52px",
    height: "52px",
    objectFit: "contain",
  }} />
              </div>
              <span style={{fontSize:"10px",fontWeight:700,color:"#6b7280"}}>{q.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* ── Next date nudge ── */}
      {nextLetter && (
        <div className="db-c2 tap-card" onClick={() => setTab("dates")} style={{
          background:`linear-gradient(135deg,${primary}0d,${secondary}0d)`,
          borderRadius:"20px",padding:"14px 16px",
          marginBottom:"12px",
          border:`1.5px solid ${primary}30`,
          display:"flex",alignItems:"center",justifyContent:"space-between",
          boxShadow:`0 4px 16px ${primary}12`,
        }}>
          <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
            <div style={{
              ...
              display:"flex",alignItems:"center",justifyContent:"center",
              fontFamily:"'Playfair Display',serif",fontSize:"22px",fontWeight:700,color:"white",
              boxShadow:`0 4px 12px ${primary}44`,
            }}>{nextLetter}</div>
            <div>
              <div style={{fontSize:"10px",color:"#9ca3af",fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase"}}>Up next</div>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:"15px",fontWeight:700,color:"#1f2937",marginTop:"2px"}}>
                Plan your <span className="shimmer-t">"{nextLetter}" date</span>
              </div>
            </div>
          </div>
          <div style={{fontSize:"20px",color:primary,opacity:0.6}}>›</div>
        </div>
      )}

      {/* ── Score duel ── */}
      <div className="db-c3 tap-card" onClick={() => setTab("betgame")} style={{
        ...
        <div style={{padding:"14px 16px"}}>
          <div style={{fontSize:"10px",color:"#9ca3af",fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"12px"}}>🏆 Score Duel</div>
          <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
            {/* P1 */}
            <div style={{flex:1,textAlign:"center"}}>
              {user?.photo
                ? <img src={user.photo} alt="" style={{width:"38px",height:"38px",borderRadius:"50%",border:"2px solid #d1fae5",objectFit:"cover",margin:"0 auto 6px",display:"block"}}/>
                : <div style={{width:"38px",height:"38px",borderRadius:"50%",background:"linear-gradient(135deg,#10b981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",color:"white",fontWeight:700,margin:"0 auto 6px"}}>{p1Name[0]}</div>
              }
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:"26px",fontWeight:700,color:"#10b981",lineHeight:1}}>{p1Score}</div>
              <div style={{fontSize:"11px",color:"#6b7280",marginTop:"3px",fontWeight:600}}>{p1Name}</div>
            </div>
            {/* VS */}
            <div style={{flexShrink:0,textAlign:"center"}}>
              <div style={{padding:"6px 10px",background:"#f0fdf4",borderRadius:"10px",fontSize:"12px",fontWeight:800,color:"#10b981",border:"1.5px solid #d1fae5"}}>VS</div>
              <div style={{fontSize:"10px",color:"#9ca3af",marginTop:"6px"}}>
                {p1Score===p2Score ? "🤝 Tied" : p1Score>p2Score ? `${p1Name} leads` : `${p2Name} leads`}
              </div>
            </div>
            {/* P2 */}
            <div style={{flex:1,textAlign:"center"}}>
              {partner?.photo
                ? <img src={partner.photo} alt="" style={{width:"38px",height:"38px",borderRadius:"50%",border:"2px solid #d1fae5",objectFit:"cover",margin:"0 auto 6px",display:"block"}}/>
                : <div style={{width:"38px",height:"38px",borderRadius:"50%",background:"linear-gradient(135deg,#059669,#10b981)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",color:"white",fontWeight:700,margin:"0 auto 6px"}}>{p2Name[0]}</div>
              }
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:"26px",fontWeight:700,color:"#10b981",lineHeight:1}}>{p2Score}</div>
              <div style={{fontSize:"11px",color:"#6b7280",marginTop:"3px",fontWeight:600}}>{p2Name}</div>
            </div>
          </div>
        </div>
      </div>

      {/* ── Progress row: Dates + Bucket ── */}
      <div className="db-c4" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"12px"}}>
        {/* Dates */}
        <div className="tap-card" onClick={() => setTab("dates")} style={{
          background:"white",borderRadius:"20px",overflow:"hidden",
          border:`1.5px solid ${primary}28`,
          boxShadow:`0 4px 16px ${primary}0a`,
        }}>
          <div style={{height:"4px",background:`linear-gradient(90deg,${primary},${secondary})`}}/>
          <div style={{padding:"14px 12px"}}>
            <div style={{fontSize:"10px",color:"#9ca3af",fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"8px"}}>📅 Alphabet Dates</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:"30px",fontWeight:700,color:primary,lineHeight:1,marginBottom:"4px"}}>
              {totalDone}<span style={{fontSize:"14px",color:"#e5e7eb",fontWeight:400}}>/26</span>
            </div>
            <div style={{height:"5px",background:`${primary}18`,borderRadius:"3px",overflow:"hidden",marginBottom:"6px"}}>
              <div style={{height:"100%",width:`${Math.round((totalDone/26)*100)}%`,background:`linear-gradient(90deg,${primary},${secondary})`,borderRadius:"3px",transition:"width 1.2s ease"}}/>
            </div>
            <div style={{fontSize:"10px",color:"#9ca3af"}}>
              {totalDone===26 ? "🎉 All done!" : `${26-totalDone} left`}
            </div>
          </div>
        </div>
        {/* Bucket */}
        <div className="tap-card" onClick={() => setTab("bucket")} style={{
          background:"white",borderRadius:"20px",overflow:"hidden",
          border:"1.5px solid #c4b5fd28",
          boxShadow:"0 4px 16px rgba(139,92,246,0.06)",
        }}>
          <div style={{height:"4px",background:"linear-gradient(90deg,#8b5cf6,#a78bfa)"}}/>
          <div style={{padding:"14px 12px"}}>
            <div style={{fontSize:"10px",color:"#9ca3af",fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"8px"}}>🌟 Bucket</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:"30px",fontWeight:700,color:"#8b5cf6",lineHeight:1,marginBottom:"4px"}}>
              {bucketDone}<span style={{fontSize:"14px",color:"#e5e7eb",fontWeight:400}}>/{buckets.length}</span>
            </div>
            <div style={{height:"5px",background:"#ede9fe",borderRadius:"3px",overflow:"hidden",marginBottom:"6px"}}>
              <div style={{height:"100%",width:`${buckets.length?Math.round((bucketDone/buckets.length)*100):0}%`,background:"linear-gradient(90deg,#8b5cf6,#a78bfa)",borderRadius:"3px",transition:"width 1.2s ease"}}/>
            </div>
            <div style={{fontSize:"10px",color:"#9ca3af"}}>
              {buckets.length===0?"Add dreams!": `${buckets.length-bucketDone} to go`}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Latest memory ── */}
      {latestMem && (
        <div className="db-c5 tap-card" onClick={() => setTab("memories")} style={{
          ...
          {latestMem.imageUrl && (
            <div style={{height:"130px",overflow:"hidden",position:"relative"}}>
              ...
              <div style={{position:"absolute",bottom:"10px",left:"14px",right:"14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{fontSize:"13px",fontWeight:700,color:"white",fontFamily:"'Playfair Display',serif"}}>{latestMem.title}</div>
                <span style={{fontSize:"20px"}}>{latestMem.mood||"🥰"}</span>
              </div>
            </div>
          )}
          <div style={{padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div>
              <div style={{fontSize:"10px",color:"#9ca3af",fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase"}}>📸 Latest Memory</div>
              {!latestMem.imageUrl && <div style={{fontSize:"13px",fontWeight:700,color:"#1f2937",marginTop:"3px"}}>{latestMem.title} {latestMem.mood||"🥰"}</div>}
              <div style={{fontSize:"11px",color:"#9ca3af",marginTop:"3px"}}>{memories.length} total · tap to see all</div>
            </div>
            <div style={{fontSize:"20px",color:"#fb923c",opacity:0.6}}>›</div>
          </div>
        </div>
      )}

      {/* ── Truth or Dare nudge ── */}
      <div className="db-c6 tap-card" onClick={() => setTab("truthordare")} style={{
        background:`linear-gradient(135deg,${primary}0d,${secondary}0d)`,
        borderRadius:"20px",padding:"14px 16px",
        border:`1.5px solid ${primary}25`,
        display:"flex",alignItems:"center",justifyContent:"space-between",
        boxShadow:`0 4px 14px ${primary}0f`,
      }}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          <div style={{
            ...
            background:`linear-gradient(135deg,${primary},${secondary})`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:"22px",boxShadow:`0 4px 12px ${primary}44`,flexShrink:0,
          }}>💬</div>
          <div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:"15px",fontWeight:700,color:"#1f2937"}}>Truth or Dare</div>
            <div style={{fontSize:"11px",color:"#9ca3af",marginTop:"2px"}}>Play a round together 🔥</div>
          </div>
        </div>
        <div style={{fontSize:"20px",color:primary,opacity:0.6}}>›</div>
      </div>

      <p style={{textAlign:"center",fontSize:"11px",color:"#fda4af",marginTop:"22px",fontStyle:"italic",fontFamily:"'Playfair Display',serif"}}>
